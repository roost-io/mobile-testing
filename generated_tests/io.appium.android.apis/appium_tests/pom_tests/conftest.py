...
def driver():
    ...
    # For remote servers, the app is assumed to be pre-installed via ADB.
    ...
    # Device unlock (if lock screen is configured)
    ...
    # Retry session creation — first attempt often fails on cloud emulators
    # because Appium installs helper APKs which temporarily disconnects UiAutomation
    d = None
    last_err = None
    ...
    # Start screen recording for test evidence
    try:
    ...
    try:
        ...
        d.press_keycode(3)  # Android HOME key
    except Exception:
        pass
    d.quit()
